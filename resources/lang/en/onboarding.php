<?php

return [

    'modes' => [
        'tour' => 'Guided tour',
        'changelog' => 'Changelog',
        'hint' => 'Hints (beacons)',
    ],

    'target_types' => [
        'data_tour' => 'Tour target (data-tour)',
        'css' => 'CSS selector',
        'none' => 'No target (centred)',
    ],

    'placements' => [
        'auto' => 'Auto',
        'top' => 'Top',
        'right' => 'Right',
        'bottom' => 'Bottom',
        'left' => 'Left',
    ],

    'resource' => [
        'label' => 'Tour',
        'plural_label' => 'Onboarding tours',
        'sections' => [
            'basics' => 'Tour',
            'publishing' => 'Publishing',
            'audience' => 'Audience',
            'audience_help' => 'Leave everything empty to show the tour to every user. Each filled criterion must be satisfied.',
        ],
        'fields' => [
            'name' => 'Name',
            'key' => 'Key',
            'key_help' => 'Unique identifier used in code, exports and the preview URL.',
            'description' => 'Description',
            'mode' => 'Mode',
            'version' => 'Version',
            'version_help' => 'Bump this to show the tour again to users who already saw it.',
            'route_pattern' => 'Route pattern',
            'route_pattern_help' => 'Path the tour runs on, e.g. admin/orders*. Wildcards allowed, comma-separate several. Empty = every page.',
            'is_active' => 'Active',
            'is_published' => 'Published',
            'published_at' => 'Published at',
            'starts_at' => 'Starts at',
            'ends_at' => 'Ends at',
            'sort' => 'Sort',
            'audience_roles' => 'Roles',
            'audience_permissions' => 'Permissions',
            'audience_segments' => 'Segments',
            'audience_segments_help' => 'Reusable audiences defined in the config or the plugin; the user must belong to at least one.',
            'tenant_id' => 'Tenant',
            'tenant_id_help' => 'Leave empty to show in every tenant.',
            'record_path' => 'Page',
            'record_path_help' => 'The panel page to record on, e.g. admin/orders. You can also paste its full URL.',
            'record_path_invalid' => 'Enter a page of this app, without wildcards.',
            'steps_count' => 'Steps',
            'completions_count' => 'Seen by',
        ],
        'actions' => [
            'preview' => 'Preview',
            'record_steps' => 'Record steps',
            'record_steps_description' => 'The route pattern matches more than one page. Enter the page to record the steps on.',
            'record_new_tour' => 'Record a tour',
            'record_new_tour_description' => 'Creates an unpublished tour for this page and opens it in record mode, where you click the elements to highlight.',
            'record_submit' => 'Open record mode',
            'reset_seen_state' => 'Reset seen-state',
            'reset_seen_state_confirm' => 'Every user will see this tour again on their next visit.',
            'reset_seen_state_done' => 'Seen-state reset for :count user(s).',
        ],
        'steps' => [
            'title' => 'Steps',
            'label' => 'step',
            'fields' => [
                'title' => 'Title',
                'target_type' => 'Target type',
                'target' => 'Target',
                'target_data_tour' => 'Tour target key',
                'target_data_tour_help' => 'The key passed to ->tourTarget(\'key\') on the Filament component.',
                'target_css' => 'CSS selector',
                'target_css_help' => 'Fragile: prefer a tour target key. Must match exactly one element.',
                'body' => 'Body',
                'placement' => 'Placement',
                'advance_on_click' => 'Advance when the element is clicked',
                'advance_on_click_help' => 'Lets the user click the highlighted element (a wire:click button, a link…) and moves to the next step once Livewire has settled.',
                'before' => 'Open this first',
                'before_help' => 'Actions run before the step is shown, e.g. click the button that opens the modal or tab containing the target.',
                'before_add' => 'Add action',
                'before_type' => 'Action',
                'before_target' => 'Target',
                'before_target_help' => 'A tour target key, or a CSS selector.',
                'before_timeout' => 'Timeout',
            ],
            'before_types' => [
                'click' => 'Click element',
                'wait' => 'Wait for element',
            ],
        ],
    ],

    'recorder' => [
        'title' => 'Record tour',
        'pick' => 'Pick element',
        'picking' => 'Click an element… (Esc to cancel)',
        'picking_hint' => '↑ selects the parent, ↓ goes back to the child.',
        'centred' => '+ Centred step',
        'empty' => 'No steps yet. Pick an element on the page to add the first step.',
        'preview' => 'Preview',
        'save' => 'Save',
        'saving' => 'Saving…',
        'saved' => 'Saved',
        'save_failed' => 'Saving failed',
        'exit' => 'Exit',
        'minimise' => 'Minimise',
        'confirm_exit' => 'You have unsaved steps. Leave record mode anyway?',
        'step_title' => 'Title',
        'step_body' => 'Body',
        'step_placement' => 'Placement',
        'target' => 'Target',
        'no_target' => 'No target (centred)',
        'retarget' => 'Re-pick',
        'add_step' => 'Add step',
        'update_step' => 'Update step',
        'cancel' => 'Cancel',
        'edit' => 'Edit',
        'remove' => 'Remove',
        'unsaved' => 'Unsaved changes',
        'before' => 'Open this first',
        'before_pick' => '+ Pick element to click first',
        'before_help' => 'Click these before the step is shown (e.g. the button that opens the modal).',
        'advance_on_click' => 'Advance when the element is clicked',
    ],

    'translations' => [
        'heading' => 'Translations',
    ],

    'analytics' => [
        'heading' => 'Analytics',
        'version' => 'Current version :version',
        'all_versions' => 'All versions',
        'show_all' => 'Show all versions',
        'show_current' => 'Show current version only',
        'empty' => 'No data yet.',
        'steps' => 'Per step',
        'missing_targets' => 'Missing targets',
        'missing_targets_hint' => 'These selectors could not be found when the tour ran. Add ->tourTarget() to the component or fix the selector.',
        'stats' => [
            'views' => 'Views',
            'unique_viewers' => 'Unique viewers',
            'completed' => 'Completed',
            'dismissed' => 'Dismissed',
            'completion_rate' => 'Completion rate',
        ],
        'columns' => [
            'step' => 'Step',
            'reached' => 'Reached',
            'reached_rate' => 'Reached %',
            'drop_off' => 'Drop-off',
            'selector' => 'Selector',
            'count' => 'Times',
            'last_seen' => 'Last seen',
        ],
    ],

    'changelog' => [
        'heading' => "What's new",
        'trigger' => "What's new",
        'got_it' => 'Got it',
    ],

    'hints' => [
        'got_it' => 'Got it',
        'dismiss_all' => 'Dismiss all hints',
        'open' => 'Open hint: :title',
    ],

    'overlay' => [
        'next' => 'Next',
        'previous' => 'Back',
        'done' => 'Done',
        'close' => 'Close',
        'progress' => 'Step :current of :total',
    ],

];
